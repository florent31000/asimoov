...
class FutureResolver:
    ...
    # ASIMOOV PATCH: drop a pending future so a timed-out request does not
    # leak an entry in pending_callbacks / chunk_data_storage forever.
    def cancel_resolve(self, message_type, topic, identifier):
        key = self.generate_message_key(message_type, topic, identifier)
