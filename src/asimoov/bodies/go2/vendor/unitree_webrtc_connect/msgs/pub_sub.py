...
class WebRTCDataChannelPubSub:
    ...
    async def publish(self, topic, data=None, msg_type=None):
        ...
        future = asyncio.get_running_loop().create_future()  # ASIMOOV PATCH: was get_event_loop()
    ...
    # ASIMOOV PATCH: counterpart of publish_request_new for a caller that gave
    # up (asyncio.wait_for timeout). Without it the future stays in
    # future_resolver.pending_callbacks for the lifetime of the connection.
    def cancel_request(self, topic, identifier, msg_type=None):
        self.future_resolver.cancel_resolve(
            msg_type or DATA_CHANNEL_TYPE["REQUEST"], topic, identifier
        )
