...
class UnitreeWebRTCConnection:
    ...
    async def connect(self):
        ...
        if self.connectionMethod == WebRTCConnectionMethod.Remote:
            ...
            if not self.ip and self.sn:
                # ASIMOOV PATCH: blocking multicast scan off the event loop.
                discovered_ip_sn_addresses = await asyncio.to_thread(discover_ip_sn)
    ...
    async def get_answer_from_remote_peer(self, pc, turn_server_info):
        ...
        # ASIMOOV PATCH: blocking HTTP off the event loop.
        peer_answer_json = await asyncio.to_thread(
            send_sdp_to_remote_peer, self.sn, json.dumps(sdp_offer_json), self.token, self.public_key
        )
    ...
    async def get_answer_from_local_peer(self, pc, ip):
        ...
        # ASIMOOV PATCH: blocking HTTP off the event loop, so the caller's
        # asyncio.wait_for around connect() can actually fire.
        peer_answer_json = await asyncio.to_thread(send_sdp_to_local_peer, ip, json.dumps(sdp_offer_json))
